...
#                                     conn.execute("INSERT OR IGNORE INTO journals (email, journal_name) VALUES (?, ?)", journal)
...
# def update_start_end(email, start, end):
#     conn.execute("UPDATE users SET start=?, end=? WHERE email=?", (start, end, email))
#     conn.commit()
...
#                         # Update last sent date and save new journals to database
#                         conn.execute("UPDATE users SET last_sent_date=? WHERE email=?", (datetime.now(), email))
#                         conn.executemany("INSERT OR IGNORE INTO journals (email, journal_name) VALUES (?, ?)", new_journals)
#                         conn.commit()

#                         # Update start and end based on the number of journals sent
#                         start += journals_sent
#                         end += journals_sent
#                         update_start_end(email, start, end)
#                     except Exception as e:
#                         logging.error(f"Error sending email to: {email}. Error: {str(e)}")
#         else:
#             logging.info(f"No new journals to send for user: {email}")
